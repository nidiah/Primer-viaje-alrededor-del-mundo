...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <!-- variables globales -->
    <xsl:variable name="out_dir" select="'pigafetta_pages'"/>
    <xsl:variable name="places_json" select="'pigafettaPlaces.js'"/>
    ...
    <xsl:template match="tei:body/tei:div/tei:div/tei:div">
        ...
        <!--xsl:result-document method="text" encoding="utf-8"
            ...
            <xsl:text>book: </xsl:text><xsl:value-of select="../../tei:head[1]/text()"/><xsl:text>&#x0A;</xsl:text>
            <xsl:text>section: </xsl:text><xsl:value-of select="preceding-sibling::tei:head/text()"/><xsl:text>&#x0A;</xsl:text>
            <xsl:text>subsection: </xsl:text><xsl:value-of select="tei:head/text()"/><xsl:text>&#x0A;</xsl:text>
            <xsl:text>type: texto&#x0A;</xsl:text>
            <xsl:text>img: </xsl:text><xsl:value-of select="position()"/><xsl:text>&#x0A;</xsl:text>
            <xsl:text>---&#x0A;</xsl:text>
            
            <xsl:text>&lt;div class="row"&gt;&#x0A;</xsl:text>
            <xsl:text>&lt;h2 class="mb-3"&gt;{{page.section}}&lt;/h2&gt;&#x0A;</xsl:text>
            <xsl:text>&lt;h3 class="mb-3"&gt;{{page.subsection}}&lt;/h3&gt;&#x0A;</xsl:text>
            ...
            <xsl:text>&lt;a class="no-underline" href="{{site.baseurl}}/assets/img/{{page.img}}.png"&gt;&lt;img src="{{site.baseurl}}/assets/img/{{page.img}}.png"/&gt;&lt;/a&gt;&#x0A;</xsl:text>
            <xsl:text>&lt;/div&gt;&#x0A;</xsl:text>
            <xsl:text>&lt;/div&gt;&#x0A;</xsl:text>
    ...
    <!--xsl:template match="tei:p">
        <xsl:text>&lt;p&gt;</xsl:text>
        <xsl:apply-templates select="tei:persName | tei:placeName | tei:span | text()"/>
        <xsl:text>&lt;/p&gt;&#x0A;</xsl:text>
    </xsl:template-->
